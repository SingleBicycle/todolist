...
const MODEL =
  process.env.VLM_MODEL || "meta-llama/llama-3.2-90b-vision-instruct";
...
app.post("/api/eval-handwriting", async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res
        .status(500)
        .json({ error: "server_config", detail: "Missing OPENROUTER_API_KEY" });
    }
...
Score how well the handwritten image matches the target.`;
    ...
    if (!resp.ok) {
      ...
      return res
        .status(502)
        .json({ error: "VLM upstream error", detail: text });
    }
